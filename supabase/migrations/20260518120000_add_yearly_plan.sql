-- 1년 이용권(yearly) 추가
-- orders.plan CHECK constraint에 'yearly' 추가
...
ALTER TABLE public.orders ADD CONSTRAINT orders_plan_check
  CHECK (plan IN ('free_trial', 'monthly', 'full_package', 'lifetime', 'yearly'));

-- create_license RPC에 yearly (365일) 추가
...
BEGIN
  ...
  v_expires_at := CASE p_plan
    WHEN 'free_trial'   THEN NOW() + INTERVAL '5 days'
